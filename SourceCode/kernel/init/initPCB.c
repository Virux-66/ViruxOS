...
PUBLIC void initializePCB() {
	...
	for (int i = 0; i < processNumber; i++) {
		...
		pPCB->ldt[0].attr1 = DA_P | (PRIVILEGE_RING0 << 5) | DA_S | DA_C_X | DA_C_R;
		memcpy(&pPCB->ldt[1], &gdt[2], sizeof(Descriptor));
		pPCB->ldt[1].attr1 = DA_P | (PRIVILEGE_RING0 << 5) | DA_S | DA_D_0 | DA_D_W;
		//pPCB->ldt[1].limitHigh_attr2 =0xcf;
		//initialize segment register,selector connected to LDT
		pPCB->stackframe.cs = 0x00;
		pPCB->stackframe.cs = SA_LDT | SA_RPL0;
		pPCB->stackframe.ds = 0x00;
		pPCB->stackframe.ds = 0x08 | SA_LDT | SA_RPL0;
		pPCB->stackframe.es = 0x00;
		pPCB->stackframe.es = 0x08 | SA_LDT | SA_RPL0;
		pPCB->stackframe.ss = 0x00;
		pPCB->stackframe.ss = 0x08 | SA_LDT | SA_RPL0;
		pPCB->stackframe.gs = 0x00;
		pPCB->stackframe.gs = (SELECTOR_VIDEO-0x03) | SA_GDT | SA_RPL3;//!!
		pPCB->stackframe.fs = 0x00;
		pPCB->stackframe.fs = 0x08 | SA_LDT | SA_RPL0;
